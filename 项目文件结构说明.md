...
- `background.js`：扩展后台 Service Worker 入口壳，负责模块装配、初始化、全局常量、少量保留的领域函数与运行入口；当前已按 `activeFlowId` 装配 flow-aware 的步骤定义、执行注册表、自动运行与状态同步，OAuth 后链 5 分钟总预算只由 `openai + cpa` target capability 自动启用，其他来源与 flow 不启用该预算；`openai` flow 根据显式 `targetId / accountDeliveryMode / accountDeliveryRouteId` 组合注册、暂存支付和账号交付节点，Plus 在持久化与运行入口继续强制关闭，并支持 `webchat / chatgpt2api` 两类 OpenAI 私有远程上传 target 在注册成功等待后直接上传当前 ChatGPT 会话；`kiro` flow 走独立的“注册页 1-6 步 -> 桌面授权 7-8 步 -> `kiro.rs` 上传 9 步”链路，`grok` flow 走独立的“注册页 1-4 步 -> SSO Cookie 提取 5 步 -> `webchat2api` 上传 6 步”链路。
...
- `flows/openai/background/session-reader.js`：OpenAI 通用 ChatGPT 会话读取器，负责在自动化窗口锁内选择并登记 `openai-session` 标签、注入只读内容脚本、按调用方声明的 `session / accessToken` 必需字段执行有界就绪重试；无窗口锁时才兼容回退到当前窗口和全局 ChatGPT 标签。
- `flows/openai/background/agent-identity.js`：OpenAI Agent Identity 协议模块，负责解析 accessToken claims、生成和导出 Ed25519 密钥、编码 OpenSSH public key、注册 OpenAI Agent，并在内存中构造不含原始 accessToken 的 `auth.json`。
...
- `background/sub2api-api.js`：SUB2API 管理接口直连模块，负责后台登录 SUB2API、解析 openai 分组、按配置选择代理、生成 OpenAI OAuth 地址、交换 localhost callback 授权码并创建账号；ChatGPT Session 与 Agent Identity 共用 Codex 凭据预检、导入 payload、结果计数和敏感错误脱敏边界，主链路不依赖 SUB2API 后台页面 DOM 自动化。
...
- `flows/openai/background/steps/wait-registration-success.js`：步骤 6 实现，负责注册资料提交后等待 7 秒，让注册成功状态和页面跳转稳定；默认不清理 cookies，只有侧栏第六步 `清 Cookies` 开关开启时才会在等待结束后清理 ChatGPT / OpenAI 相关 cookies。
...
- `flows/openai/background/steps/cpa-session-import.js`：CPA 的 ChatGPT Session 账号交付步骤，依赖通用 session reader 获取完整会话，再调用 `background/cpa-api.js` 直接导入 CPA；不再自行选择标签、注入脚本或依赖 Plus checkout 协议。
- `flows/openai/background/steps/sub2api-agent-identity-import.js`：SUB2API Agent Identity 交付步骤，严格按 SUB2API 预检、读取 accessToken、注册单个 Agent、导入同一内存 `auth.json` 的顺序执行；注册后的临时导入错误只复用当前对象重试一次，不会重新读会话、生成密钥或注册 Agent。
...
- `flows/openai/background/steps/sub2api-session-import.js`：SUB2API 的 ChatGPT Session 账号交付步骤，依赖通用 session reader 获取完整会话，再调用 `background/sub2api-api.js` 的 `codex-session` 导入接口直接创建或更新账号。
...
- `flows/openai/content/chatgpt-session.js`：ChatGPT 会话只读内容脚本，只响应后台的 `OPENAI_SESSION_GET_CURRENT` 协议，请求当前页面 `/api/auth/session` 并返回 `session / accessToken`；不加载操作延迟模块，也不包含任何 checkout DOM 或支付逻辑。
...
- `data/step-definitions.js`：共享步骤元数据，前后台共同使用，用于动态渲染、workflow 生成和步骤注册；当前 `openai` flow 会按注册方式、暂存支付方式和 capability 已解析的账号交付 route 生成连续编号的组合 workflow，`kiro` flow 则提供独立的 9 节点注册/桌面授权/上传 workflow。

## `docs/`

- `docs/仓库协作者AI分析PR与合并标准流程.md`：仓库协作者进行 AI 分析 PR 与合并时的流程说明。
- `docs/superpowers/specs/2026-07-22-openai-account-delivery-design.md`：OpenAI 账号交付架构升级设计，定义按目标保存的交付方式、target capability、阶段式 workflow、通用会话读取、Agent Identity 安全边界与分阶段自检门禁。
...
- `flows/openai/account-delivery.js`：OpenAI 账号交付方式注册表，统一定义 OAuth、ChatGPT Session、Agent Identity 的稳定 ID、标签、说明和归一化规则；目标支持矩阵与实际 route 由 OpenAI target capability 声明。
- `flows/openai/index.js`：OpenAI flow 定义，声明 `CPA / SUB2API / Codex2API / webchat / ChatGPT2API` target、OpenAI 注册与远程上传 runtime source、可见设置分组、driver 定义和 target capability；每个 target 的 capability 是账号交付方式支持列表、默认方式与 route 映射的唯一事实源，`webchat / ChatGPT2API` 属于 OpenAI flow 私有远程上传 target，不启用 OpenAI 接码配置，也不作为贡献 adapter 自动发布。
- `flows/openai/workflow.js`：OpenAI workflow 定义，按“注册阶段 -> 可选支付阶段 -> 账号交付阶段”组合节点，并统一重新生成连续的 `id / order`；账号交付只消费 capability 已解析的 route，CPA 支持 OAuth/Session，SUB2API 支持 OAuth/Session/Agent Identity，Codex2API 固定 OAuth，`webchat / ChatGPT2API` 固定 Session，不再导出支付方式乘交付方式的 workflow 变体。
...
- `core/flow-kernel/flow-capabilities.js`：flow 能力矩阵归一化模块，负责根据 `activeFlowId` 与来源解析 sidepanel 的显隐能力、来源作用域、贡献模式边界、步骤范围 UI 作用域、OpenAI `webchat / ChatGPT2API` 上传配置校验，以及按 target 归一账号交付方式的可选项、默认值、最终 route、显隐和运行锁；目标不支持请求方式时会回退到该 target 的有效方式并返回结构化校验错误。
...
- `core/flow-kernel/settings-schema.js`：统一设置 schema v6 与归一化层，负责把持久配置收敛到 `settingsState.services.*` 与 `settingsState.flows.*` 结构；OpenAI 账号交付方式按 `flows.openai.targets.<targetId>.accountDeliveryMode` 独立保存，旧 Plus 策略和 SUB2API 导入方式只在迁移入口读取并从规范化结果中清除，同时维护 `activeFlowId`、OpenAI `webchat / ChatGPT2API` target 配置、Kiro/Grok 的 `flows.<flowId>.targetId / targets` 与 `stepExecutionRangeByFlow`。
...
- `sidepanel/account-delivery-control.js`：OpenAI 账号交付选择器的纯 UI 控制器，只根据 capability 输出渲染目标可用方式、说明、显隐与禁用状态，并在变更时回传显式 `targetId / accountDeliveryMode`；目标矩阵、默认方式和 route 不在侧栏重复维护。
...
- `sidepanel/i18n-static.js`：侧边栏静态国际化映射，负责把 HTML 静态文本、title、aria-label、placeholder 与步骤标题中的中文文案映射到英文；当前包含 OpenAI 账号交付方式说明、`ChatGPT2API` 上传状态与上传步骤标题映射。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  码或从 QQ / 网易 / Gmail 转发目标邮箱收码；当邮箱服务或邮箱生成器为 Cloud Mail 时，额外显示 API 地址、管理员账号、接收邮箱和生成域名配置行；当邮箱服务为 YYDS Mail 时，额外显示 API Key 与 Base URL 配置行，并隐藏普通邮箱生成/转发邮箱相关配置；来源下拉框当前支持 `CPA / SUB2API / Codex2API / webchat / ChatGPT2API`，来源选择后紧接独立的“账号交付”行，多方式目标显示 capability 提供的选项，单一方式目标隐藏；其中 Codex2API 额外提供后台地址和管理密
...
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、共享验证码自动重发次数配置与广播接收，并装
  ...
  在普通失败时继续复用当前邮箱，只有成功或出现手机号验证时才切换下一个邮箱；OpenAI 账号交付方式按目标读取与保存，快速切换目标时以事件携带的 target 更新对应 canonical 状态，陈旧异步回包不会切回当前来源；运行锁、设置锁和贡献模式由 capability 控制编辑权限，手动与自动入口共用同一组合 workflow；Plus capability 继续固定关闭，当前选中的 PayPal 账号仍可同步回兼容字段 `paypalEmail / paypalPassword`，供暂存的底层步骤复用；Step 8 的自定义邮箱确认弹
...
- `tests/background-cpa-session-import.test.js`：测试 CPA Session 交付步骤只通过共享 reader 获取完整会话，并覆盖日志、目标 API 结果透传和后台接线约束。
...
- `tests/background-account-delivery-mode.test.js`：测试 schema v6 的账号交付迁移优先级与旧字段清理、三阶段 workflow route 组合、Agent Identity 执行器接线，以及运行锁和显式 target 保存约束。
- `tests/sidepanel-account-delivery-control.test.js`：测试账号交付纯 UI 控制器按 capability 渲染可用方式、说明、显隐和禁用状态，变更事件携带显式 target，以及销毁后不再响应交互。
...
- `tests/background-openai-session-reader.test.js`：测试通用 ChatGPT session reader 的 URL 白名单、标签优先级、自动化窗口锁、通用消息协议、必需字段和最多 11 次就绪重试边界。
- `tests/background-openai-agent-identity.test.js`：测试 Agent Identity 的 JWT claims、Ed25519/OpenSSH/PKCS#8 编码、Agent 注册请求、超时与远端错误脱敏。
- `tests/background-sub2api-agent-identity-import.test.js`：测试 Agent Identity 步骤的预检顺序、不可逆注册边界、同对象导入重试、Stop 检查和瞬态错误白名单。
- `tests/background-sub2api-session-import.test.js`：测试 SUB2API Session 交付的管理接口登录、分组/代理选择、共享 reader 与 Codex 导入边界、结果解析、敏感内容脱敏和后台接线约束。
- `tests/openai-chatgpt-session-content.test.js`：测试 ChatGPT 只读 session 内容脚本的消息来源约束、`/api/auth/session` 请求和协议错误返回。
...
- `tests/flow-capabilities-module.test.js`：测试 flow 能力矩阵模块对来源作用域、显隐能力、账号交付方式/route、OpenAI `ChatGPT2API` 配置校验和贡献模式边界的归一化。
- `tests/flow-registry-settings-schema.test.js`：测试 flow registry 与 settings schema v6 之间的默认值、来源映射、按目标账号交付偏好、OpenAI `ChatGPT2API` 独立配置和 flow-specific 配置结构保持一致。
...
- `tests/sidepanel-flow-source-registry.test.js`：测试 sidepanel 的 flow/source 选择器、Kiro 来源切换、OpenAI 账号交付控件装配、跨目标状态隔离、陈旧回包保护、`ChatGPT2API` 配置接线与显隐分组接线。
...
- `tests/step-definitions-module.test.js`：测试共享步骤定义模块、flow-specific workflow 输出、OpenAI 注册/支付/账号交付三阶段组合、连续节点编号与侧边栏脚本加载顺序。
