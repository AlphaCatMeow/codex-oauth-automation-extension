...
- `openai`：OpenAI / ChatGPT 注册与 OAuth 链路、按 target 选择账号交付方式，以及 `webchat / ChatGPT2API` 私有远程会话上传链路；Plus 支付底层暂存但当前不可用
...
[sidepanel/sidepanel.html](./sidepanel/sidepanel.html) + [sidepanel/sidepanel.js](./sidepanel/sidepanel.js) + [sidepanel/account-delivery-control.js](./sidepanel/account-delivery-control.js) + [sidepanel/account-pool-ui.js](./sidepanel/account-pool-ui.js) + [sidepanel/update-service.js](./sidepanel/update-service.js) + [sidepanel/contribution-content-update-service.js](./sidepanel/contribution-content-update-service.js)
...
- Plus capability 当前关闭，侧栏隐藏 Plus 开关和支付配置；PayPal、无卡直绑与无需支付的底层步骤暂存，不会从用户界面或启动消息进入。OpenAI 账号交付控件独立于 Plus，按当前 target capability 显示或隐藏。
...
- OpenAI 的 target capability 统一决定账号交付方式：`cpa` 支持 `oauth / session`，`sub2api` 支持 `oauth / session / agent_identity`，`codex2api` 固定 `oauth`，`webchat / chatgpt2api` 固定 `session`
...
[data/step-definitions.js](./data/step-definitions.js) 提供共享步骤元数据，并按当前 `activeFlowId` 输出 flow-specific workflow。`openai` flow 会按 `signupMethod / targetId / accountDeliveryMode / accountDeliveryRouteId` 解析步骤标题与节点集合，统一组合“注册 -> 可选 dormant 支付 -> 账号交付”三段；Session route 直接替换 OAuth 尾链，Agent Identity route 只在 SUB2API target 可用；当 target 为 `webchat` 或 `chatgpt2api` 时，会省略 OAuth 登录、登录验证码、后置手机号、OAuth 确认与平台验证尾链，并追加对应的会话上传节点；`kiro` flow 输出固定的 9 节点注册/桌面授权/上传 workflow；`grok` flow 输出固定的 6 节点注册、SSO Cookie 提取与 `webchat2api` 上传 workflow。
...
- OpenAI 账号交付尾链由 `accountDeliveryRouteId` 决定：OAuth route 进入 `oauth-login -> fetch-login-code -> 按注册方式解析的手机号/绑定邮箱节点 -> confirm-oauth -> platform-verify`；Session route 由通用 session reader 读取当前会话后直接导入 CPA、SUB2API 或私有远程 target；Agent Identity route 先完成 SUB2API 预检，再注册 Agent 并导入同一内存 `auth.json`。
- `platform-verify` 只属于 OAuth route；Session 和 Agent Identity route 不进入 OAuth callback 状态机，也不在失败时静默回退到其他交付方式。
...
- `fetch-login-code` 固定的验证码页显示邮箱 `step8VerificationTargetEmail`（字段名保留兼容）
- 当前手机号验证激活记录 `currentPhoneActivation`
- 可复用的手机号验证激活记录 `reusablePhoneActivation`
- localhost 回调地址
- SUB2API OAuth 运行态：`sub2apiSessionId / sub2apiOAuthState / sub2apiGroupId / sub2apiGroupIds / sub2apiDraftName / sub2apiProxyId`，用于把 `oauth-login` 生成的会话、目标分组与代理选择传给 `platform-verify` 的后台直连接口。
- 自动运行轮次信息
...
- 当前 flow 与目标配置：`activeFlowId`、OpenAI 的 canonical `settingsState.flows.openai.selectedTargetId`、Kiro/Grok 的 `flows.<flowId>.targetId`；`panelMode` 只保留兼容读取投影
...
- SUB2API 配置区提供“测试并同步分组”：使用当前地址、邮箱和密码完成后台登录，再按当前 flow 请求 `openai` 或 `grok` 分组；测试成功后用服务端返回的真实分组刷新当前下拉并持久化，未返回可用分组时会明确阻止同步。
- Codex2API 配置
...
- Plus 模式开关 `plusModeEnabled`（读取、导入、保存和启动时固定归一为 `false`）
- OpenAI 各 target 的账号交付方式 `settingsState.flows.openai.targets.<targetId>.accountDeliveryMode`；当前选中 target 的平铺视图只作为兼容读取投影
- Plus 支付方式 `plusPaymentMethod` 当前只保留 `paypal / paypal-hosted / none`，仅作为 dormant payment stage 配置。
...
7. 当主流程进入 `oauth-login` 时，后台改为调用公开接口 `POST https://flowpilot.qlhazycoder.top/oauth/api/start` 申请贡献登录地址，而不是去 CPA / SUB2API / Codex2API 来源刷新 OAuth
8. `oauth-login` 拿到 `session_id / auth_url / state` 后，继续沿用原有登录链路进入授权页
9. `confirm-oauth` 负责捕获 localhost callback；贡献模式下后台也会持续监听导航变化，必要时提前兼容处理 callback
10. `platform-verify` 在贡献模式下不再打开 CPA 管理页，而是围绕公开贡献会话做 callback 提交兼容和最终状态确认；当状态进入 `auto_approved / auto_rejected / manual_review_required / expired / error` 时结束
...
1. 固定等待 7 秒，等待注册成功状态和页面跳转稳定
2. 默认不删除 ChatGPT / OpenAI cookies
3. 如果侧栏第六步 `清 Cookies` 开关已开启，则在等待结束后清理 ChatGPT / OpenAI 相关 cookies
4. 完成后进入 capability 解析出的账号交付 route；OAuth 进入 `oauth-login`，Session / Agent Identity 直接进入各自交付节点

### OAuth 账号交付：`oauth-login`
...
3. 根据统一账号标识选择登录身份：
   - 邮箱账号：沿用现有邮箱登录链路
   - 手机号账号：优先探测登录页是否存在手机号登录入口，必要时展开更多选项或从邮箱页切到手机号页；进入手机号输入页后会复用注册输入节点的可视国家下拉框同步逻辑，按本轮手机号最长区号切到目标国家，填写后还会读回输入框确认已包含本地号码，避免英国 `+44` 号码仍按默认澳大利亚 `+61` 提交或只提交区号
4. 登录；如果进入密码页且当前有密码，则填写并提交密码；如果当前没有密码但检测到一次性验证码入口，则直接切换到一次性验证码登录
5. 确保真正进入验证码页或登录后续页；手机号登录允许以 `phone-verification` 作为 `oauth-login` 的成功落点，手机号注册后的 OAuth 登录也允许以真实 `add-email` 完成并交给 `fetch-login-code` 接管
6. 如果页面没有可用的手机号登录入口，则 `oauth-login` 直接抛出明确业务错误，不再误进入验证码轮询
7. 如果未进入验证码页，则按可恢复逻辑最多重试 3 次；但一旦当前失败已经明确进入 `https://auth.openai.com/add-phone`，则立即退出当前节点内部重试，不再继续第 2 / 3 次尝试
8. 自动运行进入 OAuth retry group 后，普通可恢复错误会按 workflow 锚点回到 `oauth-login`；Session / Agent Identity route 不进入该 retry group
9. localhost callback 由 `confirm-oauth` 捕获并交给 `platform-verify` 提交，不再根据固定可见步骤号跳过 OAuth 节点

贡献模式补充：

- 贡献模式下，`oauth-login` 不再从 CPA / SUB2API / Codex2API 来源刷新 OAuth，而是直接调用公开贡献接口 `/oauth/api/start`
- 贡献接口返回的 `auth_url` 会写回运行态 `oauthUrl`，后续 `oauth-login / fetch-login-code / confirm-oauth` 继续复用现有授权链路

### OAuth 账号交付：`fetch-login-code`
...
3. 当前是邮箱验证码页：
   - 对非 2925 provider，固定当前验证码页显示邮箱为本节点的目标邮箱
   - 打开邮箱页或 API 轮询入口
   - 轮询登录验证码并回填
...
6. 如遇邮箱轮询类失败或显式的 `STEP8_RESTART_STEP7` 兼容恢复错误，则按有限次数回到 `oauth-login`；若仍停在验证码页、手机验证码页或 add-email 页，则优先在当前链路重试
7. 获取到登录验证码后不再前置回放 `oauth-login`，直接在当前验证码页提交并进入后续手机号验证或 `confirm-oauth` 节点

补充：

- 对 `2925 provide` 而言，`fetch-login-code` 不依赖验证码页显示邮箱做收件匹配，而是直接测试所有命中 ChatGPT / OpenAI 过滤条件的邮件。
- 对 `2925 receive` 而言，`fetch-login-code` 会把当前目标注册邮箱一并传给 2925 内容脚本；只有当邮件里显式写出了其他邮箱时才会跳过，从而在不破坏历史兼容性的前提下，尽量降低误收验证码的概率。
- 对 `custom` provider 而言，`fetch-login-code` 仍使用手动验证码确认弹窗；弹窗当前额外提供“出现手机号验证”按钮，点击后会直接抛出与真实 `add-phone` 页面一致的 fatal 错误，供 Auto 按既有 add-phone 分支继续下一邮箱。
- 当登录验证码提交后进入 `phone-verification` 页时，内容脚本会显式把该页面识别为“手机验证码页”，避免与邮箱验证码页混淆。
- 手机号注册身份本身不会让 `fetch-login-code` 自动走短信；只有真实页面状态是 `phone_verification_page` 时才调用短信 helper。手机号注册登录后出现 `add-email` 时，本节点走“按需生成邮箱 -> 绑定邮箱 -> 邮箱验证码”链路。
- 如果手机号注册账号在 `add-email` / 邮箱验证码阶段失败并回退到 `oauth-login`，`oauth-login` 会优先使用当前轮保留的注册手机号身份重新登录，避免误用尚未绑定成功的邮箱登录。
- `email_in_use` 会在当前节点内仅清空 `registrationEmailState.current / state.email`，保留 `registrationEmailState.previous` 作为上一轮比较基线，再重新打开 `add-email` 获取新邮箱；`max_check_attempts` 不继续点击认证页 `重试`，只恢复当前节点。

### OAuth 账号交付：`post-login-phone-verification`
...
1. 读取当前认证页状态；如果已经进入 OAuth 同意页，直接完成并跳过手机号验证
2. 只有真实 `add-phone / phone-verification` 页面才进入 OpenAI 专属手机号验证流程；未开启接码时返回明确错误
3. 按 `phoneSmsProvider` 选择 HeroSMS / 5sim / NexSMS，并使用对应 API Key、国家/地区和价格上限申请或复用号码
4. 后台按 `currentPhoneActivation.provider` 分发查码、完成、取消、ban 和 reuse；验证码失败或超时按平台规则换号，不无条件重跑整个账号交付 route
5. 手机号验证完成后把当前 workflow 节点标记完成，再进入 `confirm-oauth`
...
2. 恢复或打开当前认证标签页，等待 OAuth 同意页内容脚本就绪
3. 按内容脚本与 debugger 策略多轮定位并点击“继续”，每轮都受 Stop 和 OAuth 总预算约束
4. 一旦捕获 localhost callback，写入 `localhostUrl` 并完成当前节点

### OAuth 账号交付：`platform-verify`
...
- 本节描述的是 OAuth 账号交付 route 的最终 `platform-verify` 节点，可见步骤号由当前 workflow 动态解析。
- Session 和 Agent Identity route 不进入这里；它们由各自的账号交付 executor 直接完成导入。只有 capability 解析为 OAuth route 时，才会执行本节的 callback 校验和平台创建。

流程：

1. 校验 localhost callback 是否有效
2. 判断当前来源是 CPA、SUB2API 还是 Codex2API
3. CPA、SUB2API 与 Codex2API 优先走各自的后台协议分支，不依赖目标后台页面 DOM
4. 提交回调地址或等价的授权码交换请求；SUB2API 会调用 `/api/v1/admin/openai/exchange-code` 交换授权码，再调用 `/api/v1/admin/accounts` 创建账号
5. 按目标协议校验结构化成功响应；遗留页面兼容路径只有在出现非错误态的精确成功标记且没有可见失败提示时才判定成功
6. 识别 `认证失败:*`、`认证失败: timeout of 30000ms exceeded`、`回调 URL 提交失败: oauth flow is not pending` 等失败提示并立即报错
7. 完成平台侧验证；消息路由在确认该节点是当前 workflow 最终节点后追加账号运行历史成功记录
8. 做成功后的清理与标记

Codex2API 补充：

- `oauth-login` 直接调用 `POST /api/admin/oauth/generate-auth-url` 获取 `auth_url / session_id`
- 授权页仍沿用现有 OpenAI 登录、验证码、OAuth 同意页与 localhost callback 主链
- `platform-verify` 直接调用 `POST /api/admin/oauth/exchange-code`，用 callback 中的 `code / state` 完成账号创建
- Codex2API 这条来源不新增 panel content script，也不依赖“添加账号 -> OAuth 授权 -> 生成授权链接”页面按钮 DOM

SUB2API 补充：

- `oauth-login` 直接调用 `POST /api/v1/auth/login` 登录管理接口，再调用 `/api/v1/admin/groups/all` 校验目标 openai 分组；如果配置了默认代理，会读取 `/api/v1/admin/proxies/all?with_count=true` 并按名称或 ID 选择代理。
- `oauth-login` 调用 `POST /api/v1/admin/openai/generate-auth-url` 获取 `auth_url / session_id / state`，并把目标分组 ID、代理 ID、草稿账号名写入运行态。
- `platform-verify` 会直接用 localhost callback 中的 `code / state` 调用 `POST /api/v1/admin/openai/exchange-code`，再用返回的 OpenAI token 信息调用 `POST /api/v1/admin/accounts` 创建 SUB2API 账号。
- OpenAI 的 OAuth、ChatGPT Session 与 Agent Identity 交付，以及 Grok 的 SUB2API OAuth 创建，都会显式使用账号并发 `1`。
- SUB2API 主链路不再打开后台页面，也不再依赖 `flows/openai/content/sub2api-panel.js` 的 DOM 自动化；该内容脚本仅保留为旧后台页面路径的兼容能力。

贡献模式补充：

- 贡献模式下，`platform-verify` 不再打开 CPA 管理页
- `platform-verify` 会先尝试提交已捕获的 callback URL，随后轮询公开贡献状态，直到进入最终态
...
OpenAI 账号交付已经从 Plus 概念中独立出来。用户先选择 `openai` flow 和 target，再在 capability 允许时选择交付方式；偏好保存于 `settingsState.flows.openai.targets.<targetId>.accountDeliveryMode`，实际执行 route 由 capability 统一解析。

目标支持矩阵：

- `CPA`：`oauth`、`session`
- `SUB2API`：`oauth`、`session`、`agent_identity`
- `Codex2API`：固定 `oauth`
- `webchat`：固定 `session`
- `ChatGPT2API`：固定 `session`

三类交付 route：

1. `OAuth`
   - 进入 `oauth-login -> fetch-login-code -> 按注册方式解析的手机号/绑定邮箱节点 -> confirm-oauth -> platform-verify`
   - `platform-verify` 按当前 target 调用 CPA、SUB2API 或 Codex2API 的 OAuth 接入协议
2. `ChatGPT Session`
   - 通过通用 `flows/openai/background/session-reader.js` 锁定当前 automation window 的已登录 ChatGPT 标签页
   - 读取 `/api/auth/session`，按 executor 声明的必需字段等待有限次数
   - CPA / SUB2API 直接导入会话；`webchat / ChatGPT2API` 直接上传会话
3. `Agent Identity`
   - 仅 SUB2API 可用
   - 先完成 SUB2API 登录、目标分组和代理预检，再读取 accessToken 并生成 Ed25519 Agent Identity
   - 注册成功后使用同一个内存 `auth.json` 导入；导入瞬态错误只在步骤内部按白名单重试，不重新注册 Agent

workflow 始终按以下组合生成：

```text
注册阶段 -> 可选 dormant 支付阶段 -> 账号交付阶段
```

交付阶段只消费 `accountDeliveryRouteId`，不生成支付方式与交付方式的交叉 workflow。Session / Agent Identity route 直接完成导入，不进入 OAuth callback 或 `platform-verify`，也不会在失败时静默回退到其他方式。手动执行、自动运行、手动跳过、完成判断和 OAuth retry group 都使用同一份组合 workflow。

侧栏行为：

- 多方式 target 显示账号交付下拉、当前方式说明和可编辑状态。
- 单一方式 target 隐藏控件，避免提供无效选择。
- 快速切换 target 时，每个 target 的偏好独立恢复；异步保存或 `DATA_UPDATED` 的陈旧回包只更新其携带的 target，不会把当前选择切回旧 target。
- workflow 运行、设置锁定和贡献模式期间控件不可编辑；贡献模式固定使用 OAuth，但不覆盖已保存的 target 偏好。

安全边界：

- ChatGPT accessToken、Agent Identity 私钥和完整敏感 `auth.json` 只在内存协议边界使用，不写入 `chrome.storage`、运行态快照、日志或错误文本。
- Agent Identity 生成前必须完成 SUB2API 预检；生成后不得静默降级到 Session。
- Session reader 的候选标签页优先限制在 sidepanel 启动时锁定的 automation window，避免跨窗口读取错误会话。

### Dormant Plus payment stage

Plus 当前不可用：`plusModeEnabled` 在读取、导入、保存、自动启动和后台入口均强制为 `false`，侧栏不显示 Plus 开关或支付配置。PayPal、Hosted 和免支付底层节点仍保留，支付方式只允许 `paypal / paypal-hosted / none`，未来恢复时只能作为上图中的可选 payment stage 插入，不能重新把账号交付挂回 Plus 或复制交叉 workflow。
...
5. 执行 `runAutoSequenceFromStep`
   ...
   - `oauth-login` 内部仍保留登录态恢复的有限重试，但 `add-phone / 手机号页` 属于立即跳出的不可重试错误
   - `fetch-login-code` 若在验证码提交后进入 `add-phone / 手机号页`，会直接抛出 fatal 错误，不再先标记节点成功
   - OAuth retry group 遇到普通报错且认证流程未进入 `add-phone` 时，会按 workflow 锚点回到 `oauth-login`；Session / Agent Identity route 不进入这条恢复分支
   ...
   - 如果是手动停止，则立即退出自动流程，不会再触发“回到 `oauth-login` 重开”
6. 如果失败，根据设置决定：
   - 立即停止
   - 当前轮重试
   - 下一轮继续
...
7. 是否可以不打开来源后台页面，直接把 `oauth-login / platform-verify` 收敛到协议分支
...
- `oauth-login` 在识别到登录超时报错页时，会先通过共享恢复逻辑最多自动点击 5 次 `重试` 尝试恢复当前页面；恢复成功后会优先按当前页面状态继续当前登录流程，例如直接续跑邮箱页或密码页；只有仍未恢复到可继续状态时，才按原有可恢复失败逻辑重跑 `oauth-login`。
- `oauth-login` 在首次识别到登录验证码页后，不会立刻把节点判定为成功；还会额外做一轮“收尾确认”，确保页面稳定停留在登录验证码页。如果只是短暂进入验证码页、随后又掉进登录重试页，则会先走共享恢复逻辑，再按既有可恢复失败逻辑重跑 `oauth-login`。
- `oauth-login` 的这轮收尾确认是主要责任边界；`fetch-login-code` 默认建立在“登录验证码页已经稳定确认”的前提上，只在后台入口保留防御性回退判断，不替代前序节点收尾。
- `fetch-login-code` 如果发现认证页已经进入登录超时报错/重试页，会直接报错并回到 `oauth-login` 重新开始，而不是在当前节点内点击 `重试`。
- `fetch-login-code` 的登录重试页判定也覆盖 `/email-verification` 上的 `405 / Route Error`，避免这类页面被误当成普通未知页。
...
- `confirm-oauth` 在点击 OAuth 同意页 `继续` 后，会持续等待页面跳转；若点击后命中认证页重试页，则直接报错，不会在当前节点内点击 `重试`。
